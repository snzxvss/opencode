  ...
  init: (props: { mode: "dark" | "light"; source?: ThemeSource }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "claude-code")
        draft.active = typeof active === "string" ? active : "claude-code"
        draft.ready = false
      }),
    )
    ...
    function syncCustomThemes() {
      return themes
        ...
        .catch(() => setStore("active", "claude-code"))
    }
...
if (store.active === "system") setStore("active", "claude-code")
            return
          }
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes["claude-code"] ?? store.themes.opencode, store.mode)
    })
